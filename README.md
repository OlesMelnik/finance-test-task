...
- не написано тестів (це обов’язково згідно опису завдання) - Added Tests ✔️
- зміна даних через хук useState в файле App.js працює, але це робиться неправильно - якщо дані залежать від попереднього значенні змінної group, то потрібно використовувати коллбек а не вставляти значення напряму
в коді є невікорістани змінні - Fixed ✔️
- в файлі GroupBadge у відкриваючий HTML тег передано обробник на подію click - це працює але це неправильний підхід - правильнише передавати посилання на функцію - Fixed ✔️
- в коді зустрічається коментований код без такої потреби - Fixed ✔️
- в коді змішано використовується MaterialUI та звичйний CSS - це працює, але доброюпрактикою є використанно чогось одного - якщо прийнято рішення використовувати якийсь фреймворк: то правильно його використовувати максимально, якщо прийнято рішення користуватись звичайним CSS, то використовується CSS, бажано послуговуватись в такому випадку модульним підходом (можна поєднувати з препроцессором та БЕМ, або без такого) - Fixed ✔️
